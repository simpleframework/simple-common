...
public abstract class PageTagSupport extends PagerTagSupport {
	...
	protected final void setPageAttributes(final int page) {
		if (pageTagExport == null) {
			...
			final int maxPageItems = pagerTag.getMaxPageItems();

			if ((name = pageTagExport.getFirstItem()) != null) {
				final int firstItem = (page * maxPageItems) + 1;
				pageContext.setAttribute(name, new Integer(firstItem));
			}

			if ((name = pageTagExport.getLastItem()) != null) {
				final int lastItem = Math.min((page + 1) * maxPageItems, pagerTag.getItemCount());
				pageContext.setAttribute(name, new Integer(lastItem));
			}
		}
	}

	protected final void setOffsetAttributes(final int offset) {
		if (pageTagExport == null) {
			...
			if ((name = pageTagExport.getFirstItem()) != null) {
				final int firstItem = offset + 1;
				pageContext.setAttribute(name, new Integer(firstItem));
			}

			if ((name = pageTagExport.getLastItem()) != null) {
				final int lastItem = Math.min(offset + maxPageItems, pagerTag.getItemCount());
				pageContext.setAttribute(name, new Integer(lastItem));
			}
		}
	}
