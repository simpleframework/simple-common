...
public class MultipartParser {
	...
	public MultipartParser(final HttpServletRequest req, final int maxSize, final boolean buffer,
		...
		if (length > maxSize) {
			throw new IOException(
					"Posted content length of " + length + " exceeds limit of " + maxSize);
		}

		// Get the boundary string; it's included in the content type.
		// Should look something like "------------------------12012133613061"
		final String boundary = extractBoundary(type);
		if (boundary == null) {
	...
	private String[] extractDispositionInfo(String line) throws IOException {
		...
		final String name = origline.substring(start + startOffset, end);
