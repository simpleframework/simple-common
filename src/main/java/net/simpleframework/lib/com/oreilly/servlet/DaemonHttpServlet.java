...
public abstract class DaemonHttpServlet extends HttpServlet {
	...
	public void init(final ServletConfig config) throws ServletException {
		...
		try {
			...
			log("Problem starting socket server daemon thread" + e.getClass().getName() + ": "
					+ e.getMessage());
		}
	}
	...
	public void destroy() {
		try {
			...
			log("Problem stopping server socket daemon thread: " + e.getClass().getName() + ": "
					+ e.getMessage());
		}
	}
}
...
class Daemon extends Thread {
	...
	private final DaemonHttpServlet servlet;

	public Daemon(final DaemonHttpServlet servlet) {
	...
	public void run() {
		try {
			...
			servlet.log("Problem establishing server socket: " + e.getClass().getName() + ": "
					+ e.getMessage());
			return;
		}
		...
		try {
			...
			try {
				...
				servlet.log("Problem closing server socket: " + ioe.getClass().getName() + ": "
						+ ioe.getMessage());
			}
		}
	}
