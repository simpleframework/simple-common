...
public final class StringUtil {
	...
	public static String join(final Iterator strings, final String sep) {
		...
		final StringBuilder sb = StringUtil.borrowBuilder().append(start);
	...
	public static boolean isInvisibleChar(final int c) {
		...
		// previously also included zw non join, zw join - but removing those
		// breaks semantic meaning of text
	}
	...
	/**
	 * Maintains cached StringBuilders in a flyweight pattern, to minimize new
	 * StringBuilder GCs. The StringBuilder is
	 * prevented from growing too large.
	 * <p>
	 * Care must be taken to release the builder once its work has been
	 * completed, with {@link #releaseBuilder}
	 * 
	...
	/**
	 * Release a borrowed builder. Care must be taken not to use the builder
	 * after it has been returned, as its
	 * contents may be changed by this method, or by a concurrent thread.
	 * 
	 * @param sb
	 *        the StringBuilder to release.
	 * @return the string value of the released String Builder (as an incentive
	 *         to release it!).
	 */
	public static String releaseBuilder(StringBuilder sb) {
		...
		if (sb.length() > MaxCachedBuilderSize) {
			sb = new StringBuilder(MaxCachedBuilderSize); // make sure it hasn't
																			// grown too big
		} else {
