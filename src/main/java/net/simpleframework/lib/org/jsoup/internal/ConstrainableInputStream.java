...
/**
 * A jsoup internal class (so don't use it as there is no contract API) that
 * enables constraints on an Input Stream,
...
public final class ConstrainableInputStream extends BufferedInputStream {
	...
	/**
	 * If this InputStream is not already a ConstrainableInputStream, let it be
	 * one.
	 * 
	 * @param in
	 ...
	 * @param maxSize
	 *        the maximum size to allow to be read. 0 == infinite.
	 * @return a constrainable input stream
	 */
	public static ConstrainableInputStream wrap(final InputStream in, final int bufferSize,
			final int maxSize) {
		return in instanceof ConstrainableInputStream ? (ConstrainableInputStream) in
	...
	public int read(final byte[] b, final int off, int len) throws IOException {
		...
		if (Thread.interrupted()) {
			// interrupted latches, because parse() may call twice (and we still
			// want the thread interupt to clear)
			interrupted = true;
			return -1;
		}
		if (expired()) {
	...
	/**
	 * Reads this inputstream to a ByteBuffer. The supplied max may be less than
	 * the inputstream's max, to support
	 * reading just the first bytes.
	 */
	public ByteBuffer readToByteBuffer(final int max) throws IOException {
		Validate.isTrue(max >= 0, "maxSize must be 0 (unlimited) or larger");
		final boolean localCapped = max > 0; // still possibly capped in total
															// stream
		...
		while (true) {
			...
			if (localCapped) { // this local byteBuffer cap may be smaller than the
										// overall maxSize (like when reading first bytes)
				if (read >= remaining) {
