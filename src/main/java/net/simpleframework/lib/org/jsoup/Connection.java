...
public interface Connection {
	 ...
	 */
	enum Method {
		GET(false), POST(true), PUT(true), DELETE(false), PATCH(true), HEAD(false), OPTIONS(
				false), TRACE(false);
	 ...
	 *        user-agent to use
	 * @return this Connection, for chaining
	 * @see net.simpleframework.lib.org.jsoup.helper.HttpConnection#DEFAULT_UA
	 */
	 ...
	 * and the input truncated (i.e. the body content will be trimmed). <b>The
	 * default maximum is 2MB</b>. A max size of
	 * <code>0</code> is treated as an infinite amount (bounded only by your
	 * patience and the memory available on your
	 * machine).
	 *
	 * @param bytes
	...
	interface Request extends Base<Request> {
		 ...
		 * 
		 * @param proxy
		 *        the proxy ot use; <code>null</code> to disable.
		 * @return this Request, for chaining
		 */
