...
public class Tag implements Cloneable {
	 ...
	 * @param settings
	 *        used to control tag name sensitivity
	...
	public static Tag valueOf(String tagName, final ParseSettings settings) {
		...
		if (tag == null) {
			...
			final String normalName = Normalizer.lowerCase(tagName); // the
																						// lower-case
			// name to get tag
			// settings off
			...
			if (tag == null) {
				...
				tag = tag.clone(); // get a new version vs the static one, so name
											// update doesn't reset all
				tag.tagName = tagName;
			}
		}
		return tag;
	 ...
	 *
	 * @param tagName
	 *        Name of tag, e.g. "p". <b>Case sensitive</b>.
	 * @return The tag, either defined or new generic.
	 */
	...
	// todo - rework this to format contents as inline; and update html emitter
	// in Element. Same output, just neater.
	...
	private static final String[] formSubmitTags = { "input", "keygen", "object", "select",
			"textarea" };
	...
	static {
		...
		for (final String tagName : formListedTags) {
			final Tag tag = tags.get(tagName);
			Validate.notNull(tag);
			tag.formList = true;
		}

		for (final String tagName : formSubmitTags) {
			final Tag tag = tags.get(tagName);
			Validate.notNull(tag);
			tag.formSubmit = true;
		}
