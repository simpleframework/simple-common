...
public final class DataUtil {
	...
	static Document parseInputStream(InputStream input, String charsetName, final String baseUri,
			final Parser parser) throws IOException {
		if (input == null) {
		...
		if (charsetName == null) { // determine from meta. safe first parse as
			...
			if (foundCharset != null && !foundCharset.equalsIgnoreCase(defaultCharset)) { // need
																													// to
																													// re-decode.
																													// (case
																													// insensitive
	...
	private static BomCharset detectCharsetFromBom(final ByteBuffer byteData) {
		final Buffer buffer = byteData; // .mark and rewind used to return Buffer,
													// now ByteBuffer, so cast for backward
													// compat
		buffer.mark();
