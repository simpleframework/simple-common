...
/**
 * Node filter interface. Provide an implementing class to {@link NodeTraversor}
 * to iterate through nodes.
 * <p>
 * This interface provides two methods, {@code head} and {@code tail}. The head
 * method is called when the node is first
 * seen, and the tail method when all of the node's children have been visited.
 * As an example, head can be used to
 ...
 * </ul>
 * The difference between {@link FilterResult#SKIP_CHILDREN} and
 * {@link FilterResult#SKIP_ENTIRELY} is that the first
 * will invoke {@link NodeFilter#tail(Node, int)} on the node, while the latter
 * will not.
 * Within {@link NodeFilter#tail(Node, int)}, both are equivalent to
 * {@link FilterResult#CONTINUE}.
 * </p>
 */
public interface NodeFilter {
	...
	enum FilterResult {
		...
		CONTINUE,
		/**
		 * Skip the child nodes, but do call {@link NodeFilter#tail(Node, int)}
		 * next.
		 */
		SKIP_CHILDREN,
		/**
