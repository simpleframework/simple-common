...
/**
 * Standard exception thrown for all general compileShared and some runtime
 * failures.
 */
public class CompileException extends RuntimeException {
	...
	public CompileException(final String message, final List<ErrorDetail> errors, final char[] expr,
		...
		if (!errors.isEmpty()) {
			final ErrorDetail detail = errors.iterator().next();
	...
	private CharSequence showCodeNearError(final char[] expr, final int cursor) {
		...
		do {
			...
			final int matchIndex = match == null ? 0 : cs.indexOf(match);
