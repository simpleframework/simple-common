...
public class LineMapper {
	...
	public LineMapper(final char[] expr) {
		this.expr = expr;
	}
	...
	public LineLookup map() {
		...
		return new LineLookup() {
			@Override
			public int getLineFromCursor(final int cursor) {
				for (final Node n : lineMapping) {
			...
			@Override
			public boolean hasLine(final int line) {
	...
	private static class Node implements Comparable<Node> {
		...
		private Node(final int cursorStart, final int cursorEnd, final int line) {
		...
		public boolean isInRange(final int cursor) {
