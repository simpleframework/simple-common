...
import net.simpleframework.lib.org.mvel2.ast.NestedStatement;
...
import net.simpleframework.lib.org.mvel2.util.ASTIterator;
...
public class DebugTools {
	...
	public static String decompile(final Serializable expr) {
	...
	public static String decompile(final CompiledExpression cExp) {
	...
	private static String decompile(final CompiledExpression cExp, final boolean nest,
			final DecompileContext context) {
		final ASTIterator iter = new ASTLinkedList(cExp.getFirstNode());
		ASTNode tk;
		...
		while (iter.hasMoreNodes()) {
			...
			if ((tk = iter.nextNode()) instanceof NestedStatement
				...
				sbuf.append(decompile((CompiledExpression) ((NestedStatement) tk).getNestedStatement(),
						true, context));
			}
			if (tk instanceof Substatement
				...
				// noinspection StringConcatenationInsideStringBufferAppend
				sbuf.append("NEST [" + tk.getClass().getSimpleName() + "]: { " + tk.getName() + " }\n");
				sbuf.append(decompile((CompiledExpression) ((Substatement) tk).getStatement(), true,
						context));
			}
			// else if (tk instanceof Function) {
			// sbuf.append("FUNCTION [" + tk.getName() + "]: ")
			// .append(decompile((CompiledExpression)
			// ((Function)tk).getCompiledBlock(), true, context));
			// }
				...
				sbuf.append("REFERENCE :: ").append(tk.getClass().getSimpleName()).append(":")
						.append(tk.getName());
			} else if (tk instanceof BinaryOperation) {
				final BinaryOperation bo = (BinaryOperation) tk;
				sbuf.append("OPERATION [" + getOperatorName(bo.getOperation()) + "] {")
						.append(bo.getLeft().getName()).append("} {").append(bo.getRight().getName())
						.append("}");
			} else {
				// noinspection StringConcatenationInsideStringBufferAppend
	...
	public static String getOperatorSymbol(final int operator) {
		switch (operator) {
		case ADD:
			return "+";
		case SUB:
	...
	public static String getOperatorName(final int operator) {
		switch (operator) {
		case ADD:
			return "ADD";
		case SUB:
	...
	public static Map<String, VariableResolver> getAllVariableResolvers(
		...
		do {
			for (final String var : vrf.getKnownVariables()) {
