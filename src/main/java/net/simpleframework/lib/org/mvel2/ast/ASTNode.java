...
import static net.simpleframework.lib.org.mvel2.optimizers.OptimizerFactory.getDefaultAccessorCompiler;
import static net.simpleframework.lib.org.mvel2.util.CompilerTools.getInjectedImports;
import static net.simpleframework.lib.org.mvel2.util.ParseTools.handleNumericConversion;
...
public class ASTNode implements Cloneable, Serializable {
	...
	private Object deop(final Object ctx, final Object thisValue,
			final VariableResolverFactory factory, final RuntimeException e) {
		if ((fields & DEOP) == 0) {
			accessor = null;
			fields |= DEOP | NOJIT;
	...
	private Object optimize(final Object ctx, final Object thisValue,
			final VariableResolverFactory factory) {
		if ((fields & DEOP) != 0) {
			fields ^= DEOP;
		}
	...
	public void setEgressType(final Class egressType) {
	...
	private int getAbsoluteFirstPart() {
		if ((fields & COLLECTION) != 0) {
			if (firstUnion < 0 || endOfName < firstUnion) {
				return endOfName;
			} else {
				return firstUnion;
			}
	...
	public void storeInLiteralRegister(final Object o) {
		this.literal = o;
	}

	public void setLiteralValue(final Object literal) {
		this.literal = literal;
		this.fields |= LITERAL;
	}

	@SuppressWarnings({ "SuspiciousMethodCalls" })
	protected void setName(final char[] name) {
	...
	public Accessor setAccessor(final Accessor accessor) {
	...
	protected ASTNode(final ParserContext pCtx) {
		this.pCtx = pCtx;
	}

	public ASTNode(final char[] expr, final int start, final int offset, final int fields,
