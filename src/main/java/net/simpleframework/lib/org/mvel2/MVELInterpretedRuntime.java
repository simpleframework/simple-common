...
public class MVELInterpretedRuntime extends AbstractParser {
	...
	private int procBooleanOperator(final int operator) {
		switch (operator) {
		case RETURN:
			return RETURN;
		case NOOP:
	 ...
	 * @return -
	 */
	private boolean unwindStatement(final int operator) {
		ASTNode tk;

		switch (operator) {
		case AND:
			while ((tk = nextToken()) != null && !tk.isOperator(Operator.END_OF_STMT)
					&& !tk.isOperator(Operator.OR)) {
				// nothing
			}
			break;
		default:
	...
	MVELInterpretedRuntime(final char[] expression, final Object ctx,
	...
	MVELInterpretedRuntime(final String expression) {
		setExpression(expression);
		this.variableFactory = new ImmutableDefaultFactory();
	}

	MVELInterpretedRuntime(final char[] expression) {
		this.length = end = (this.expr = expression).length;
	}

	public MVELInterpretedRuntime(final char[] expr, final Object ctx,
	...
	public MVELInterpretedRuntime(final String expression, final Object ctx,
			final VariableResolverFactory resolverFactory) {
		setExpression(expression);
		this.ctx = ctx;
		this.variableFactory = resolverFactory;
	...
	public MVELInterpretedRuntime(final String expression, final Object ctx,
			final VariableResolverFactory resolverFactory, final ParserContext pCtx) {
		super(pCtx);
		setExpression(expression);
		this.ctx = ctx;
		this.variableFactory = resolverFactory;
