...
public class IndexedPostFixDecNode extends ASTNode {
	...
	public IndexedPostFixDecNode(final int register, final ParserContext pCtx) {
		super(pCtx);
	...
	public Object getReducedValueAccelerated(Object ctx, final Object thisValue,
		...
		final VariableResolver vResolver = factory.getIndexedVariableResolver(register);
		// ctx = vResolver.getValue();
		vResolver.setValue(MathProcessor.doOperations(
				ParseTools.resolveType(ctx = vResolver.getValue()), ctx, Operator.SUB,
				DataTypes.INTEGER, 1));
		return ctx;
	}
