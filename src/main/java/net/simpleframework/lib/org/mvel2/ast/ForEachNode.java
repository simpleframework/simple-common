...
public class ForEachNode extends BlockNode {
	...
	public ForEachNode(final char[] expr, final int start, final int offset, final int blockStart,
			final int blockOffset, final int fields, ParserContext pCtx) {
		super(pCtx);

		handleCond(this.expr = expr, this.start = start, this.offset = offset, this.fields = fields,
				pCtx);
	...
	public Object getReducedValueAccelerated(final Object ctx, final Object thisValue,
		...
		final ItemResolverFactory.ItemResolver itemR = new ItemResolverFactory.ItemResolver(item);
		final ItemResolverFactory itemFactory = new ItemResolverFactory(itemR,
				new DefaultLocalVariableResolverFactory(factory));

		final Object iterCond = condition.getValue(ctx, thisValue, factory);
		...
		switch (type) {
		case ARRAY:
			final int len = Array.getLength(iterCond);
			for (int i = 0; i < len; i++) {
			...
			break;
		case CHARSEQUENCE:
			for (final Object o : iterCond.toString().toCharArray()) {
				itemR.setValue(o);
				v = compiledBlock.getValue(ctx, thisValue, itemFactory);
				if (itemFactory.tiltFlag()) {
					return v;
			...
			break;
		case INTEGER:
			final int max = (Integer) iterCond + 1;
			for (int i = 1; i != max; i++) {
				itemR.setValue(i);
				v = compiledBlock.getValue(ctx, thisValue, itemFactory);
				if (itemFactory.tiltFlag()) {
		...
		case ITERABLE:
			for (final Object o : (Iterable) iterCond) {
				itemR.setValue(o);
				v = compiledBlock.getValue(ctx, thisValue, itemFactory);
				if (itemFactory.tiltFlag()) {
					return v;
	...
	public Object getReducedValue(final Object ctx, final Object thisValue,
		...
		final Object iterCond = MVEL.eval(expr, start, offset, thisValue, factory);
	...
	private void handleCond(final char[] condition, final int start, final int offset,
		...
		if ((x = (item = createStringTrimmed(condition, start, cursor - start)).indexOf(' ')) != -1) {
			final String tk = new String(condition, start, x).trim();
			try {
		...
		if ((fields & COMPILE_IMMEDIATE) != 0) {
			final Class egress = (this.condition = (ExecutableStatement) subCompileExpression(expr,
	...
	private void determineIterType(final Class t) {
	...
	private void enforceTypeSafety(final Class required, final Class actual) {
		if (!required.isAssignableFrom(actual) && !DataConversion.canConvert(actual, required)) {
			throw new CompileException("type mismatch in foreach: expected: " + required.getName()
					+ "; but found: " + getBaseComponentType(actual), expr, start);
		}
	}
}
