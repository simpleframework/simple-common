...
public class Negation extends ASTNode {
	...
	public Negation(final char[] expr, final int start, final int offset, final int fields,
		...
		if ((fields & COMPILE_IMMEDIATE) != 0) {
			if (((this.stmt = (ExecutableStatement) subCompileExpression(expr, start, offset, pCtx))
					.getKnownEgressType() != null)
					&& (!ParseTools.boxPrimitive(stmt.getKnownEgressType()).isAssignableFrom(
							Boolean.class))) {
	...
	public Object getReducedValue(final Object ctx, final Object thisValue,
		...
		try {
			...
			throw new CompileException("negation operator applied to non-boolean expression", expr,
					start, e);
		}
	}
