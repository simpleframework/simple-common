...
public class CompiledDeclareNode extends Node {
	...
	public CompiledDeclareNode(final int begin, final String name, final char[] template,
		...
		// ce = MVEL.compileExpression(this.contents = subset(template,
		// this.cStart = start, (this.end = this.cEnd = end) - start - 1),
		// context);
	}
	...
	public Object eval(final TemplateRuntime runtime, final TemplateOutputStream appender,
		...
		runtime.getNamedTemplateRegistry().addNamedTemplate(
				MVEL.executeExpression(ce, ctx, factory, String.class),
