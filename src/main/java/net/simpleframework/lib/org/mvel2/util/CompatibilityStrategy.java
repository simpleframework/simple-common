...
public class CompatibilityStrategy {
	...
	public static boolean areEqualityCompatible(final Class<?> c1, final Class<?> c2) {
		return compatibilityEvaluator.areEqualityCompatible(c1, c2);
	}

	public static boolean areComparisonCompatible(final Class<?> c1, final Class<?> c2) {
	...
	public static class DefaultCompatibilityEvaluator implements CompatibilityEvaluator {
		...
		@Override
		public boolean areEqualityCompatible(final Class<?> c1, final Class<?> c2) {
			...
			if (isBoxedNumber(c1, false) && isBoxedNumber(c2, true)) {
				return true;
			}
			if (c1.isPrimitive()) {
				return c2.isPrimitive() || arePrimitiveCompatible(c1, c2, true);
			}
			if (c2.isPrimitive()) {
				return arePrimitiveCompatible(c2, c1, false);
			}
			return false;
		}

		@Override
		public boolean areComparisonCompatible(final Class<?> c1, final Class<?> c2) {
			return areEqualityCompatible(c1, c2);
		}

		private boolean arePrimitiveCompatible(final Class<?> primitive, final Class<?> boxed,
				final boolean leftFirst) {
			if (primitive == Boolean.TYPE) {
				return boxed == Boolean.class;
			}
			if (primitive == Integer.TYPE) {
				return isBoxedNumber(boxed, leftFirst);
			}
		...
		private boolean isBoxedNumber(final Class<?> c, final boolean allowString) {
