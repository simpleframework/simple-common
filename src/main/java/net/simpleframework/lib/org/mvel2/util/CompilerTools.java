...
import net.simpleframework.lib.org.mvel2.ast.And;
import net.simpleframework.lib.org.mvel2.ast.BinaryOperation;
import net.simpleframework.lib.org.mvel2.ast.BooleanNode;
import net.simpleframework.lib.org.mvel2.ast.Contains;
import net.simpleframework.lib.org.mvel2.ast.Convertable;
import net.simpleframework.lib.org.mvel2.ast.DeclTypedVarNode;
import net.simpleframework.lib.org.mvel2.ast.Function;
import net.simpleframework.lib.org.mvel2.ast.Instance;
import net.simpleframework.lib.org.mvel2.ast.IntAdd;
import net.simpleframework.lib.org.mvel2.ast.IntDiv;
import net.simpleframework.lib.org.mvel2.ast.IntMult;
import net.simpleframework.lib.org.mvel2.ast.IntOptimized;
import net.simpleframework.lib.org.mvel2.ast.IntSub;
import net.simpleframework.lib.org.mvel2.ast.LiteralNode;
import net.simpleframework.lib.org.mvel2.ast.Or;
import net.simpleframework.lib.org.mvel2.ast.RegExMatchNode;
import net.simpleframework.lib.org.mvel2.ast.Soundslike;
import net.simpleframework.lib.org.mvel2.ast.Strsim;
...
public class CompilerTools {
	 ...
	 *        - perform a second pass optimization to optimize boolean
	 *        expressions.
	 * @param pCtx
	 *        - The parser context
	...
	public static ASTLinkedList finalizePayload(ASTLinkedList astLinkedList,
			final boolean secondPassOptimization, final ParserContext pCtx) {
			...
			if ((tk = astLinkedList.nextNode()).getFields() == -1) {
				...
				if ((tkOp = astLinkedList.nextNode()).getFields() == -1) {
					...
					if (tk.getEgressType() == Integer.class && tk2.getEgressType() == Integer.class) {
						...
						while (reduc) {
							final ASTNode oper = astLinkedList.nextNode();
							final ASTNode rightNode = astLinkedList.nextNode();

							if (rightNode == null) {
								break;
							}

							final Object val = new BinaryOperation(oper.getOperator(),
									inv ? new LiteralNode(signNumber(tk2.getLiteralValue()), pCtx) : tk2,
									rightNode, pCtx).getReducedValueAccelerated(null, null, null);
			...
			while (astLinkedList.hasMoreNodes()) {
				if ((tk = astLinkedList.nextNode()).getFields() == -1) {
					...
					if ((tkOp = astLinkedList.nextNode()).getFields() == -1) {
						...
						while (astLinkedList.hasMoreNodes()
								&& (tkOp2 = astLinkedList.nextNode()).isOperator()
								&& (tkOp2.isOperator(Operator.AND) || tkOp2.isOperator(Operator.OR))) {

							if ((tkOp = tkOp2).getOperator() == Operator.AND) {
								bool.setRightMost(new And(bool.getRightMost(), astLinkedList.nextNode(),
										pCtx.isStrongTyping(), pCtx));
							} else {
	...
	private static BinaryOperation boOptimize(final int op, final ASTNode tk, final ASTNode tk2,
	...
	private static boolean isReductionOpportunity(final ASTNode oper, final ASTNode node) {
		ASTNode n = node;
		return (n != null && n.isLiteral() && (n = n.nextASTNode) != null
				&& reducacbleOperator(n.getOperator())
				&& PTABLE[oper.getOperator()] <= PTABLE[n.getOperator()] && (n = n.nextASTNode) != null
				&& n.isLiteral() && n.getLiteralValue() instanceof Number);
	}

	private static boolean reducacbleOperator(final int oper) {
		switch (oper) {
		case Operator.ADD:
		case Operator.SUB:
			return true;
	...
	private static void optimizeOperator(final int operator, final ASTNode tk, final ASTNode tkOp,
			final ASTLinkedList astLinkedList, final ASTLinkedList optimizedAst,
			final ParserContext pCtx) {
		switch (operator) {
		case Operator.REGEX:
	...
	private static boolean isIntOptimizationviolation(final BooleanNode bn, final ASTNode bn2) {
		return (bn instanceof IntOptimized && bn2.getEgressType() != Integer.class);
	}

	public static Class getReturnType(final ASTIterator input, final boolean strongTyping) {
		final ASTNode begin = input.firstNode();
		if (begin == null) {
			return Object.class;
		}
		if (input.size() == 1) {
	...
	public static void expectType(final Accessor expression, final Class type,
		...
		if (compileMode) {
			if ((retType == null || !boxPrimitive(type).isAssignableFrom(boxPrimitive(retType)))
				...
				&& !boxPrimitive(type).isAssignableFrom(boxPrimitive(retType))) {
	...
	public static Class getReturnTypeFromOp(final int operation, final Class left, final Class right) {
		...
		case Operator.DIV:
			if (left == Object.class || right == Object.class) {
				...
				return __resolveType(boxPrimitive(left)) < __resolveType(boxPrimitive(right)) ? right
						: left;
			}
	...
	public static Accessor extractAccessor(final ASTNode n) {
	...
	public static Number signNumber(final Object number) {
