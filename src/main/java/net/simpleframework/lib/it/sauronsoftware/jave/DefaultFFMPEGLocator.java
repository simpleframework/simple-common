...
public class DefaultFFMPEGLocator extends FFMPEGLocator {
	...
	public DefaultFFMPEGLocator() {
		...
		// Temp dir?
		final File temp = new File(System.getProperty("java.io.tmpdir"), "jave-" + myEXEversion);
		if (!temp.exists()) {
			temp.mkdirs();
			temp.deleteOnExit();
		}
		// ffmpeg executable export on disk.
		final String suffix = isWindows ? ".exe" : "";
		final File exe = new File(temp, "ffmpeg" + suffix);
		if (!exe.exists()) {
			copyFile("ffmpeg" + suffix, exe);
		}
		// pthreadGC2.dll
		if (isWindows) {
			final File dll = new File(temp, "pthreadGC2.dll");
			if (!dll.exists()) {
	 ...
	 */
	private void copyFile(final String path, final File dest) throws RuntimeException {
