...
import net.simpleframework.lib.net.sf.cglib.core.ClassInfo;
...
class MethodInterceptorGenerator implements CallbackGenerator {
	...
	private static final Type REFLECT_UTILS = TypeUtils
			.parseType("net.simpleframework.lib.net.sf.cglib.core.ReflectUtils");
	private static final Type METHOD_PROXY = TypeUtils
			.parseType("net.simpleframework.lib.net.sf.cglib.proxy.MethodProxy");
	private static final Type METHOD_INTERCEPTOR = TypeUtils
			.parseType("net.simpleframework.lib.net.sf.cglib.proxy.MethodInterceptor");
	...
	private static final Signature FIND_METHODS = TypeUtils.parseSignature(
			"java.lang.reflect.Method[] findMethods(String[], java.lang.reflect.Method[])");
	private static final Signature MAKE_PROXY = new Signature("create", METHOD_PROXY,
			new Type[] { Constants.TYPE_CLASS, Constants.TYPE_CLASS, Constants.TYPE_STRING,
	...
	public void generate(final ClassEmitter ce, final Context context, final List methods) {
		...
		for (final Iterator it = methods.iterator(); it.hasNext();) {
			...
			// access method
			e = ce.begin_method(Opcodes.ACC_FINAL, impl, method.getExceptionTypes());
	...
	public void generateStatic(final CodeEmitter e, final Context context, final List methods)
		...
		final Map methodsByClass = CollectionUtils.bucket(methods, METHOD_TO_CLASS);
		for (final Iterator i = methodsByClass.keySet().iterator(); i.hasNext();) {
			final ClassInfo classInfo = (ClassInfo) i.next();

			final List classMethods = (List) methodsByClass.get(classInfo);
			e.push(2 * classMethods.size());
			e.newarray(Constants.TYPE_STRING);
			for (int index = 0; index < classMethods.size(); index++) {
				final MethodInfo method = (MethodInfo) classMethods.get(index);
				final Signature sig = method.getSignature();
				e.dup();
				e.push(2 * index);
				e.push(sig.getName());
	...
	public void generateFindProxy(final ClassEmitter ce, final Map sigMap) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC, FIND_PROXY,
				null);
		e.load_arg(0);
		...
		EmitUtils.string_switch(e, (String[]) sigMap.keySet().toArray(new String[0]),
