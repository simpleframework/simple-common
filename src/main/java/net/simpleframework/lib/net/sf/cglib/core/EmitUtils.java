...
public class EmitUtils {
	...
	public static void factory_method(final ClassEmitter ce, final Signature sig) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, sig, null);
		e.new_instance_this();
		e.dup();
		e.load_args();
	...
	public static void null_constructor(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, CSTRUCT_NULL, null);
		e.load_this();
	...
	public static void string_switch(final CodeEmitter e, final String[] strings,
			final int switchStyle, final ObjectSwitchCallback callback) {
		try {
			switch (switchStyle) {
	...
	private static void string_switch_trie(final CodeEmitter e, final String[] strings,
	...
	private static void stringSwitchHelper(final CodeEmitter e, final List strings,
			final ObjectSwitchCallback callback, final Label def, final Label end, final int index)
			throws Exception {
	 ...
	 */
	@Deprecated
	public static void hash_code(final CodeEmitter e, final Type type, final int multiplier,
	...
	private static void hash_array(final CodeEmitter e, final Type type, final int multiplier,
			final CustomizerRegistry registry) {
	...
	private static void hash_object(final CodeEmitter e, final Type type,
			final CustomizerRegistry registry) {
		// (f == null) ? 0 : f.hashCode();
		final Label skip = e.make_label();
		final Label end = e.make_label();
	...
	private static void not_equals_helper(final CodeEmitter e, final Type type,
			final Label notEquals, final CustomizerRegistry registry,
			final ProcessArrayCallback callback) {
		if (TypeUtils.isPrimitive(type)) {
	...
	private static void append_string_helper(final CodeEmitter e, final Type type,
			final ArrayDelimiters delims, final CustomizerRegistry registry,
			final ProcessArrayCallback callback) {
		final Label skip = e.make_label();
		final Label end = e.make_label();
	...
	public static void method_switch(final CodeEmitter e, final List methods,
			final ObjectSwitchCallback callback) {
		member_switch_helper(e, methods, callback, true);
	}

	public static void constructor_switch(final CodeEmitter e, final List constructors,
			final ObjectSwitchCallback callback) {
		member_switch_helper(e, constructors, callback, false);
	}

	private static void member_switch_helper(final CodeEmitter e, final List members,
			final ObjectSwitchCallback callback, final boolean useName) {
		try {
			...
			if (useName) {
				...
				EmitUtils.string_switch(e, names, Constants.SWITCH_STYLE_HASH,
						new ObjectSwitchCallback() {
							@Override
							public void processCase(final Object key, final Label dontUseEnd)
									throws Exception {
	...
	private static void member_helper_size(final CodeEmitter e, final List members,
			final ObjectSwitchCallback callback, final ParameterTyper typer, final Label def,
	...
	private static void member_helper_type(final CodeEmitter e, final List members,
			final ObjectSwitchCallback callback, final ParameterTyper typer, final Label def,
		...
		if (members.size() == 1) {
			...
			if (buckets == null || buckets.size() == 1) {
				...
				// must have two methods with same name, types, and different return
				// types
				e.goTo(def);
			} else {
				checked.set(index);
				...
				EmitUtils.string_switch(e, names, Constants.SWITCH_STYLE_HASH,
						new ObjectSwitchCallback() {
							...
							public void processCase(final Object key, final Label dontUseEnd)
									throws Exception {
								member_helper_type(e, (List) fbuckets.get(key), callback, typer, def, end,
										checked);
							}
	...
	public static void add_property(final ClassEmitter ce, final String name, final Type type,
