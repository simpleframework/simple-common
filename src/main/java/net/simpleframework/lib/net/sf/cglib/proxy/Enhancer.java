...
import net.simpleframework.lib.net.sf.cglib.core.WeakCacheKey;
 ...
 * which
 * in AOP terms enables "around advice"--that is, you can invoke custom code
 * both before
 * and after the invocation of the "super" method. In addition you can modify
 * the
 ...
 * For
 * advanced needs, such as customizing the <code>ClassLoader</code> to use, you
 * should create
 * a new instance of <code>Enhancer</code>. Other classes within CGLIB follow a
 * similar pattern.
 * <p>
 * All enhanced objects implement the {@link Factory} interface, unless
 * {@link #setUseFactory} is
 * used to explicitly disable this feature. The <code>Factory</code> interface
 * provides an API
 * to change the callbacks of an existing object, as well as a faster and easier
 * way to create
...
public class Enhancer extends AbstractClassGenerator {
	...
	/**
	 * {@link net.simpleframework.lib.net.sf.cglib.core.AbstractClassGenerator.ClassLoaderData#generatedClasses}
	 * requires to keep cache key
	 * in a good shape (the keys should be up and running if the proxy class is
	 * alive), and one of the cache keys is
	 * {@link CallbackFilter}. That is why the generated class contains static
	 * field that keeps strong reference to
	 * the {@link #filter}.
	 * <p>
	 * This dance achieves two goals: ensures generated class is reusable and
	 * available through generatedClasses
	 * cache, and it enables to unload classloader and the related
	 * {@link CallbackFilter} in case user does not need
	 * that
	 * </p>
	 */
	...
	private static final Type FACTORY = TypeUtils
			.parseType("net.simpleframework.lib.net.sf.cglib.proxy.Factory");
	...
	private static final Type CALLBACK = TypeUtils
			.parseType("net.simpleframework.lib.net.sf.cglib.proxy.Callback");
	...
	public interface EnhancerKey {
		public Object newInstance(String type, String[] interfaces,
				WeakCacheKey<CallbackFilter> filter, Type[] callbackTypes, boolean useFactory,
	 ...
	 * if the supplied superclass is actually an interface,
	 * <code>setInterfaces</code>
	 ...
	 * New object instances will always use the same mapping, but may use
	 * different
	 ...
	 * Generate a new class if necessary and uses the specified
	 * callbacks (if any) to create a new object instance.
	 * Uses the constructor of the superclass matching the
	 * <code>argumentTypes</code>
	 ...
	 * called during the constructor will not be intercepted. To avoid this
	 * problem,
	 ...
	 * {@link ReflectUtils#newInstance(Class, Class[], Object[])}
	...
	static class EnhancerFactoryData {
		...
		public EnhancerFactoryData(final Class generatedClass,
				final Class[] primaryConstructorArgTypes, final boolean classOnly) {
		...
		/**
		 * Creates proxy instance for given argument types, and assigns the
		 * callbacks.
		 * Ideally, for each proxy class, just one set of argument types should be
		 * used,
		 * otherwise it would have to spend time on constructor lookup.
		 * Technically, it is a re-implementation of
		 * {@link Enhancer#createUsingReflection(Class)},
		 * with "cache {@link #setThreadCallbacks} and
		 ...
		 */
		public Object newInstance(final Class[] argumentTypes, final Object[] arguments,
				final Callback[] callbacks) {
			setThreadCallbacks(callbacks);
			try {
				// Explicit reference equality is added here just in case
				// Arrays.equals does not have one
	...
	private static void getMethods(final Class superclass, final Class[] interfaces,
			final List methods, final List interfaceMethods, final Set forcePublic) {
	...
	public void generateClass(final ClassVisitor v) throws Exception {
		final Class sc = (superclass == null) ? Object.class : superclass;
		...
		e.declare_field(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC, FACTORY_DATA_FIELD, OBJECT_TYPE,
				null);
		...
		e.declare_field(Opcodes.ACC_PRIVATE | Opcodes.ACC_STATIC, CALLBACK_FILTER_FIELD, OBJECT_TYPE,
				null);
	 ...
	 * Technically speaking {@link #wrapCachedClass(Class)} uses
	 * {@link EnhancerFactoryData} as a cache value,
	 * and the latter enables faster instantiation than plain old reflection
	 * lookup and invoke.
	 * This method is left intact for backward compatibility reasons: just in
	 * case it was ever used.
	 *
	 * @param type
	...
	protected Object wrapCachedClass(final Class klass) {
		...
		final EnhancerFactoryData factoryData = new EnhancerFactoryData(klass, argumentTypes,
				classOnly);
		...
		try {
			...
			final Field callbackFilterField = klass.getDeclaredField(CALLBACK_FILTER_FIELD);
	...
	protected Object unwrapCachedValue(final Object cached) {
		if (currentKey instanceof EnhancerKey) {
			final EnhancerFactoryData data = ((WeakReference<EnhancerFactoryData>) cached).get();
			return data;
		}
	 ...
	 * using
	 * an instance of <code>Enhancer</code> or the {@link Factory} interface to
	 * create
	 * new instances, this method is unnecessary. Its primary use is for when you
	 * want to
	 ...
	 * Otherwise they are <i>not</i> cleared, and you should be careful to set
	 * them
	...
	private static void setCallbacksHelper(final Class type, final Callback[] callbacks,
	...
	private static Method getCallbacksSetter(final Class type, final String methodName)
	 ...
	 * Implementation detail: java.lang.reflect instances are not cached, so this
	 * method should not
	 * be used on a hot path.
	 * This method is used when {@link #setUseCache(boolean)} is set to
	 * {@code false}.
	 *
	 * @param type
	 *        class to instantiate
	...
	private void emitConstructors(final ClassEmitter ce, final List constructors) {
		...
		for (final Iterator it = constructors.iterator(); it.hasNext();) {
			final MethodInfo constructor = (MethodInfo) it.next();
	...
	private int[] getCallbackKeys() {
		final int[] keys = new int[callbackTypes.length];
		for (int i = 0; i < callbackTypes.length; i++) {
			keys[i] = i;
		}
		return keys;
	...
	private void emitGetCallback(final ClassEmitter ce, final int[] keys) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, GET_CALLBACK, null);
		e.load_this();
		e.invoke_static_this(BIND_CALLBACKS);
		e.load_this();
		e.load_arg(0);
	...
	private void emitSetCallback(final ClassEmitter ce, final int[] keys) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, SET_CALLBACK, null);
		e.load_arg(0);
		e.process_switch(keys, new ProcessSwitchCallback() {
			@Override
			public void processCase(final int key, final Label end) {
	...
	private void emitSetCallbacks(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, SET_CALLBACKS, null);
		e.load_this();
		e.load_arg(0);
		for (int i = 0; i < callbackTypes.length; i++) {
			e.dup2();
	...
	private void emitGetCallbacks(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, GET_CALLBACKS, null);
		e.load_this();
		e.invoke_static_this(BIND_CALLBACKS);
		e.load_this();
	...
	private void emitNewInstanceCallbacks(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, NEW_INSTANCE, null);
	...
	private void emitNewInstanceCallback(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, SINGLE_NEW_INSTANCE, null);
	...
	private void emitNewInstanceMultiarg(final ClassEmitter ce, final List constructors) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC, MULTIARG_NEW_INSTANCE, null);
		final Type thisType = getThisType(e);
		e.load_arg(2);
		e.invoke_static(thisType, SET_THREAD_CALLBACKS);
		e.new_instance(thisType);
	...
	private void emitMethods(final ClassEmitter ce, final List methods, final List actualMethods) {
		...
		while (it1.hasNext()) {
			final MethodInfo method = (MethodInfo) it1.next();
			final Method actualMethod = (it2 != null) ? (Method) it2.next() : null;
			final int index = filter.accept(actualMethod);
		...
		final Map bridgeToTarget = new BridgeMethodResolver(declToBridge, getClassLoader())
				.resolveAll();
		...
		final CallbackGenerator.Context context = new CallbackGenerator.Context() {
			...
			public void emitLoadArgsAndInvoke(final CodeEmitter e, final MethodInfo method) {
				...
				// invokespecial,
				// then we need to invoke_virtual w/ the bridge target instead of
				// doing
				// a super, because super may itself be using super, which would
				// bypass
				// any proxies on the target.
				final Signature bridgeTarget = (Signature) bridgeToTarget.get(method.getSignature());
	...
	private void emitSetThreadCallbacks(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC,
	...
	private void emitBindCallbacks(final ClassEmitter ce) {
		final CodeEmitter e = ce.begin_method(Constants.PRIVATE_FINAL_STATIC, BIND_CALLBACKS, null);
