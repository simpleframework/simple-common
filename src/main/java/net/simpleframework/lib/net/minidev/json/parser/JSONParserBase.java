 ...
 */
import static net.simpleframework.lib.net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_CHAR;
import static net.simpleframework.lib.net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_EOF;
import static net.simpleframework.lib.net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_LEADING_0;
import static net.simpleframework.lib.net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_TOKEN;
import static net.simpleframework.lib.net.minidev.json.parser.ParseException.ERROR_UNEXPECTED_UNICODE;
...
abstract class JSONParserBase {
	...
	public void checkLeadinZero() throws ParseException {
		...
		final char c1 = xs.charAt(0);
		final char c2 = xs.charAt(1);
		if (c1 == '-') {
			final char c3 = xs.charAt(2);
			if (c2 == '0' && c3 >= '0' && c3 <= '9') {
				throw new ParseException(pos, ERROR_UNEXPECTED_LEADING_0, xs);
			}
			return;
	 ...
	 */
	protected <T> T parse(final AMapper<T> mapper) throws ParseException {
		this.pos = -1;
		T result;
		try {
			read();
	...
	protected <T> T readArray(final AMapper<T> mapper) throws ParseException, IOException {
	 ...
	 * generated by a ContainerFactory
	 */
	protected <T> T readFirst(final AMapper<T> mapper) throws ParseException, IOException {
		for (;;) {
			switch (c) {
			// skip spaces
			case ' ':
	 ...
	 * generated by a ContainerFactory
	 */
	protected Object readMain(final AMapper<?> mapper, final boolean stop[]) throws ParseException,
			IOException {
		for (;;) {
			switch (c) {
			// skip spaces
	...
	protected <T> T readObject(final AMapper<T> mapper) throws ParseException, IOException {
		//
		if (c != '{') {
			throw new RuntimeException("Internal Error");
		}
		...
		for (;;) {
			...
			default:
				...
				readNoEnd(); /* skip : */
				lastKey = key;
				final Object value = readMain(mapper, stopValue);
	...
	public static class MSB {
		...
		public void append(final char c) {
			p++;
			if (b.length <= p) {
				final char[] t = new char[b.length * 2 + 1];
