...
public class JSONNavi<T> {
	...
	public JSONNavi<?> at(final String key) {
		...
		final Object next = o(current).get(key);
		stack.add(current);
		path.add(key);
		current = next;
		return this;
	...
	public JSONNavi<T> add(final Object... values) {
		...
		final List<Object> list = a(current);
	...
	private void store() {
		...
		if (isObject(parent)) {
			...
			final int index = ((Number) missingKey).intValue();
			final List<Object> lst = a(parent);
	...
	public JSONNavi<?> at(int index) {
		...
		@SuppressWarnings("unchecked")
		final List<Object> lst = ((List<Object>) current);
		if (index < 0) {
