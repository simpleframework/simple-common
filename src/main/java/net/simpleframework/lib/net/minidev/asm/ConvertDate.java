...
public class ConvertDate {
	...
	private static Integer parseMonth(final String s1) {
		if (Character.isDigit(s1.charAt(0))) {
			...
			final Integer month = monthsTable.get(s1);
			if (month == null) {
	...
	static {
		...
		for (final Locale locale : DateFormatSymbols.getAvailableLocales()) {
	...
	private static Date addHour(final StringTokenizer st, final Calendar cal) {
		String s1;
		if (!st.hasMoreTokens()) {
			return cal.getTime();
		}
