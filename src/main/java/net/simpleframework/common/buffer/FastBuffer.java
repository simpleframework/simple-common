...
public class FastBuffer<E> implements RandomAccess, Iterable<E> {
	...
	private void needNewBuffer(final int newSize) {
		...
		if (currentBufferIndex >= buffers.length) {
			...
			final E[][] newBuffers = (E[][]) new Object[newLen][];
	...
	public E[] toArray(int start, final int len) {
		int remaining = len;
		int pos = 0;
		final E[] array = (E[]) new Object[len];
	...
	public Iterator<E> iterator() {
		return new Iterator<E>() {
			...
			public E next() {
				...
				final E[] buf = buffers[iteratorBufferIndex];
				final E result = buf[iteratorOffset];
